window.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelector('.keyboard-eq').addEventListener('click', () => {
        if (/\d/.test(calc_display.innerText.slice(-1))) {
            set_display_text(math.evaluate(calc_display.innerText.replace(/x/g, '*').replace(/,/g, '.')))
            eq_flag = true
        }
        calc_display.scrollTo(-calc_display.clientWidth, 0)
    })

    keyboard_num.forEach(el => el.addEventListener('click', (e) => {
        if (eq_flag) set_display_text(e.target.innerText)
        else calc_display.innerText += e.target.innerText
    }))

    keyboard_op.forEach(el => el.addEventListener('click', (e) => {
        ...
        else if(/\+|-|x|\//.test(s) && s != e.target.innerText && calc_display.innerText.length > 1) {
            set_display_text(calc_display.innerText.slice(0, -1) + e.target.innerText)
        }

        else if (e.target.innerText == '-' && !calc_display.innerText) calc_display.innerText += '-'
    }))
